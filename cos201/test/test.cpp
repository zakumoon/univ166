...
int main()
{
	...
	while (true)
	{
		...
		switch (select)
		{
            ...
            break;
            case 5:{
                FILE * binWrite = fopen("/home/zk/univ166/cos201/file/writeBin.txt", "wb");

                for (int i = 0; i< n; i++){
                    fwrite(&item[i], sizeof(struct product), 1, binWrite);
                    fprintf(binWrite, "%s %s %f %d \n", 
                    item[n].id,
                    item[n].name,
                    item[n].cost,
                    item[n].instock);
            ...
            break;
            case 6: {
                FILE * binRead = fopen("/home/zk/univ166/cos201/file/readBin.txt", "rb");
        
                while (feof(binRead) == false){
                    char id[10];
                    char name[100];
                    int cost;
                    int instock;
                    ...
                    fscanf(binRead, "%s %s %f %d", id, name, &cost, &instock); 

                    int all =fread(&item[n], sizeof(struct product), 1, binRead);
                    if (all > 0){
                        cout<<"New Product"<<endl;
                        cout<<"Id? = "<<id<<endl;
                        cout<<"Name? = "<<name<<endl;
